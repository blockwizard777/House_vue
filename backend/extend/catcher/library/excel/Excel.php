...
class Excel
{
    ...
    public function save(ExcelContract $excel, $path, $disk = 'local'): array
    {
        ...
        $this->init();
        $filePath = Utils::publicPath($path);

        !is_dir($path) && mkdir($filePath, 0777, true);
        $filename = date('YmdHis').Str::random(6) . '.' .$this->extension;
    ...
    protected function url($path) {
        return \config('filesystem.disks.local')['domain'] . '/' . str_replace('\\', '/', $path);
    }
