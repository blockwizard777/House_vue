...
export function createObjectAndCopy(protoObj, dataObj) {
  ...
  let newObj = createObject(protoObj);
  for (let key in protoObj) {
    if (newObj.hasOwnProperty(key) && dataObj[key] != undefined)
...
export function createObject(protoObj) {
  return JSON.parse(JSON.stringify(protoObj))
...
export function isValidArrayIndex(array, idx) {
  if (!array || !Array.isArray(array) || !array.length > 0) return false;
  if (idx < 0 || idx >= array.length) return false;
  return true;
}
...
export function getIndexByValue(array, compare) {
...
export function convertObjectKeys(object, targetObjkey) {
  if (!isValidObject(object)) return null;
  if (!isValidObject(targetObjkey)) return null;//object;
  let ret = {};
  let tgtKeys = Object.values(targetObjkey);
  tgtKeys.map(key => {
    ret[key] = '';
  })
  let originKeys = Object.keys(object);
  return originKeys.reduce((acc, curKey) => {
    if (targetObjkey[curKey]) {
      acc[targetObjkey[curKey]] = object[curKey];
    }else {
      acc[targetObjkey[curKey]] = ''
    }
    return acc;
  }, ret)
